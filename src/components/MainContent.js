...
function MainContent() {
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://api.nasa.gov/neo/rest/v1/feed?start_date=2021-04-08&end_date=2021-04-10&api_key=DEMO_KEY`;
    fetch(apiUrl)
      .then((res) => res.json())
      .then((repos) => {
        setAppState({ loading: false, repos: repos.near_earth_objects['2021-04-08']});
      });
  }, [setAppState]);
  return (
